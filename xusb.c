...
static int test_device(uint16_t vid, uint16_t pid)
{
	...
	if (FD >= 0) {
    libusb_context *context = CONTEXT;
    int fd = FD;
    handle = NULL;
	printf("Wrap sys device %d...\n", fd);
	...
	if (FD >= 0) {
    VID = vid = dev_desc.idVendor;
    PID = pid = dev_desc.idProduct;
        }
	printf("            length: %d\n", dev_desc.bLength);
...
int main(int argc, char** argv)
{
			...
			if ( ((argv[j][0] == '-') || (argv[j][0] == '/'))
			  && (arglen >= 2) ) {
				...
				if (i != arglen) {
					...
					/* show_help = true; */
            int fd;
            if (!(sscanf(argv[j], "%d", &fd) == 1)) {
		printf("   Bad file descriptor: '%s'\n", argv[j]);
		return 1;
	    }
	    FD = fd;
	...
	if (debug_mode) {
		...
		r = libusb_init_context(/*ctx=*/&CONTEXT/* NULL */, /*options=*/&options, /*num_options=*/1);
	} else {
		r = libusb_init_context(/*ctx=*/&CONTEXT/* NULL */, /*options=*/NULL, /*num_options=*/0);
	}
